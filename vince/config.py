...
def validate_config_option(key: str, value: Any) -> None:
    ...
    if key == "version":
        if not isinstance(value, str):
            raise InvalidConfigOptionError(f"{key} (expected string, got {type(value).__name__})")
        ...
        if not isinstance(value, bool):
            raise InvalidConfigOptionError(f"{key} (expected boolean, got {type(value).__name__})")
    
    elif key == "color_theme":
        if not isinstance(value, str):
            raise InvalidConfigOptionError(f"{key} (expected string, got {type(value).__name__})")
        if value not in VALID_COLOR_THEMES:
            raise InvalidConfigOptionError(f"{key} (must be one of: {', '.join(sorted(VALID_COLOR_THEMES))})")
        ...
        if not isinstance(value, int) or isinstance(value, bool):
            raise InvalidConfigOptionError(f"{key} (expected integer, got {type(value).__name__})")
