...
def extensions(draw):
    ...
    supported = [".md", ".py", ".txt", ".js", ".html", ".css", 
...
from vince.errors import OfferExistsError, OfferNotFoundError, OfferInUseError
    ...
    *For any* offer state transition, if the transition is in the VALID_TRANSITIONS
    ...
    def test_active_to_rejected_when_in_use_raises_offer_in_use_error(self, offer_id):
        ...
        with pytest.raises(OfferInUseError) as exc_info:
            validate_offer_transition(OfferState.ACTIVE, OfferState.REJECTED, offer_id, in_use=True)
    ...
    def test_active_to_rejected_when_not_in_use_succeeds(self, offer_id):
        ...
        # Should not raise any exception
        validate_offer_transition(OfferState.ACTIVE, OfferState.REJECTED, offer_id, in_use=False)
