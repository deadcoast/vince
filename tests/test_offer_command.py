...
class TestOfferCommand:
    ...
    def test_offer_creates_offer_entry(
    ...
    def test_offer_creates_default_if_not_exists(
        self, runner, mock_executable, isolated_data_dir, monkeypatch
    ):
    ...
    def test_offer_uses_existing_default(
        self, runner, mock_executable, isolated_data_dir, monkeypatch
    ):
    ...
    def test_offer_error_when_offer_exists(
        self, runner, mock_executable, isolated_data_dir, monkeypatch
    ):
        ...
        result = runner.invoke(
            app, ["offer", "existing-offer", str(mock_executable), "--md"]
        )
    ...
    def test_offer_error_invalid_offer_id_format(
        self, runner, mock_executable, isolated_data_dir, monkeypatch
    ):
        ...
        result = runner.invoke(
            app, ["offer", "Invalid-ID", str(mock_executable), "--md"]
        )

        assert result.exit_code == 1
        assert "VE103" in result.output or "Invalid offer_id" in result.output

    def test_offer_error_reserved_name(
        self, runner, mock_executable, isolated_data_dir, monkeypatch
    ):
    ...
    def test_offer_verbose_output(
        ...
        result = runner.invoke(
            app, ["offer", "verbose-test", str(mock_executable), "--md", "-vb"]
        )

        assert result.exit_code == 0
        # Verbose output should include additional info
        assert (
            "Validating offer_id" in result.output or "Processing path" in result.output
        )

    def test_offer_error_no_extension_specified(
        self, runner, mock_executable, isolated_data_dir, monkeypatch
    ):
