...
class TestSlapListChopFlow:
    ...
    def test_slap_list_chop_complete_flow(self, runner, mock_executable, isolated_data_dir, monkeypatch):
    ...
    def test_slap_set_list_chop_flow(self, runner, mock_executable, isolated_data_dir, monkeypatch):
...
class TestSlapSetOfferAutoCreation:
    ...
    def test_slap_set_auto_creates_offer(self, runner, mock_executable, isolated_data_dir, monkeypatch):
    ...
    def test_slap_set_offer_references_correct_default(self, runner, mock_executable, isolated_data_dir, monkeypatch):
...
class TestOfferRejectFlow:
    ...
    def test_offer_reject_complete_flow(self, runner, mock_executable, isolated_data_dir, monkeypatch):
    ...
    def test_offer_reject_with_complete_delete(self, runner, mock_executable, isolated_data_dir, monkeypatch):
        ...
        # Create offer
        result = runner.invoke(app, ["offer", "delete-me", str(mock_executable), "--py"])
        assert result.exit_code == 0
    ...
    def test_multiple_offers_reject_one(self, runner, mock_executable, isolated_data_dir, monkeypatch):
        ...
        # Create two offers
        result = runner.invoke(app, ["offer", "offer-one", str(mock_executable), "--md"])
        assert result.exit_code == 0
        
        result = runner.invoke(app, ["offer", "offer-two", str(mock_executable), "--py"])
        assert result.exit_code == 0
        ...
        offers_data = json.loads((isolated_data_dir / "offers.json").read_text())
        offer_one = next(o for o in offers_data["offers"] if o["offer_id"] == "offer-one")
        offer_two = next(o for o in offers_data["offers"] if o["offer_id"] == "offer-two")
...
class TestComplexFlows:
    ...
    def test_full_lifecycle_slap_offer_reject_chop(self, runner, mock_executable, isolated_data_dir, monkeypatch):
    ...
    def test_set_forget_set_again_flow(self, runner, mock_executable, isolated_data_dir, monkeypatch):
        ...
        active_defaults = [d for d in defaults_data["defaults"] if d["state"] == "active" and d["extension"] == ".md"]
