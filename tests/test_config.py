...
def valid_config_dicts(draw):
    ...
    if draw(st.booleans()):
        config["data_dir"] = draw(st.text(min_size=1, max_size=50).filter(lambda x: x.strip()))
    if draw(st.booleans()):
...
class TestConfigPrecedence:
    ...
    *For any* configuration key that exists at multiple levels (default, user, project),
    ...
    @given(
        user_verbose=st.booleans(),
        project_verbose=st.booleans()
    )
    ...
    @given(
        user_theme=valid_color_themes(),
        project_max_backups=valid_max_backups()
    )
    @settings(max_examples=100)
    def test_partial_overrides_preserve_other_values(self, user_theme, project_max_backups):
        ...
        **Validates: Requirements 6.2, 6.3, 6.4**
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            assert merged["confirm_destructive"] == DEFAULT_CONFIG["confirm_destructive"]
        ...
        **Validates: Requirements 6.2**
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for key, default_value in DEFAULT_CONFIG.items():
                assert merged[key] == default_value, f"Key {key} should be default value"
...
class TestConfigErrorHandling:
    ...
    *For any* invalid config option value, loading SHALL raise InvalidConfigOptionError (VE401).
    ...
    @given(st.text(min_size=1, max_size=100).filter(lambda x: x.strip() and not x.strip().startswith('{')))
    @settings(max_examples=100)
